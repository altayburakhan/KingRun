...
public class GameManager : MonoBehaviour
{
    ...
    public Text warriorCostDisplay;
    public Text archerCostDisplay;
    public Text mageCostDisplay;
    public Text barbarianCostDisplay;
    private int _warriorCost = 0;
    private int _archerCost = 0;
    private int _mageCost = 0;
    private int _barbarianCost = 0;
    public GameObject selectedUnit;
    public Transform warriorUnit; // melee prefab to initiate
    public Transform archerUnit; 
    public Transform mageUnit;
    public Transform barbarianUnit;
        ...
        for (int x = 0; x < 5; x++)
        {
            for (int z = 0; z < 3; z++)
            {
                ...
                if (hitColliders.Length > 0)
                {
                    ...
                    {
                        if (hitCollider.CompareTag("Warrior") || hitCollider.CompareTag("Archer") || (hitCollider.CompareTag("Mage") || hitCollider.CompareTag("Barbarian")))
                        {
    ...
    {
        this.warriorCostDisplay.text = "Spawn Warrior: " + _warriorCost;
        this.archerCostDisplay.text = "Spawn Archer: " + _archerCost;
        this.mageCostDisplay.text = "Spawn Mage: " + _mageCost;
        this.barbarianCostDisplay.text = "Spawn Barbarian: " + _barbarianCost;
    }
        ...
        for (int i = 0; i < _gridWidth; i++)
        {
            for (int j = 0; j < _gridHeight; j++)
            {
                if (!this.grid[i, j].GetComponent<GridCell>().isOccupied)
                {
                    ...
                    {
                        Transform unit = Instantiate(archerUnit, _allySide);
                        unit.transform.localPosition = new Vector3(i, 0, j);
                        unit.transform.localScale = Vector3.one * .75f;
                        unit.name = "Archer Unit" + i + j;
                        ToggleOccupied(i, j);
                        UpdateGoldDisplay();
                        UpdateCostDisplay();
                        return;
                    ...
                    {
                        Transform unit = Instantiate(mageUnit, _allySide);
                        unit.transform.localPosition = new Vector3(i, 0, j);
                        unit.transform.localScale = Vector3.one * .75f;
                        unit.name = "Mage Unit" + i + j;
                        ToggleOccupied(i, j);
                        UpdateGoldDisplay();
                        UpdateCostDisplay();
                        return;
                    ...
                    {
                        Transform unit = Instantiate(barbarianUnit, _allySide);
                        unit.transform.localPosition = new Vector3(i, 0, j);
                        unit.transform.localScale = Vector3.one * .75f;
                        unit.name = "Barbarian Unit" + i + j;
                        ToggleOccupied(i, j);
                        UpdateGoldDisplay();
                        UpdateCostDisplay();
                        return;
